  ...
  watch(keyCodeState, (newVal, oldVal) => {
    ...
    if (isOpeningTheRecord.value) {
      ...
      if (count_down > 1) {
        ...
        // TIPS: 迁回此处原因:
        //       * 虽然在此处触发提示, 会造成isOpeningTheRecord.value为true期间, 应用未获取焦点时被意外触发。但只要控制好isOpeningTheRecord.value即可避免此意外触发的情况发生(比如在未获取焦点时, 及时的将isOpeningTheRecord.value设置为false)。
        //       * 不过最重要的原因是, 依赖前端键盘事件, 会存在某些组件阻止部分键盘事件的传递情况。
        //       * * 如本次移回就是因为 ESC 按键的up事件在传递至全局的过程中被阻止, 而造成frontendKeyEventStateBool中其使用处于按下状态, 从而使得消息不断弹出的bug。(这只是发现的, 可能还有其它按键也潜在的有这种问题)
        //       * 在此处, 由于监听的优先级是系统级别的, 可以避免实际抬起事件未能监听到的情况发生。
        //       * * 即使是,win+j 和 win+k 这类被我在第三方映射工具中, 重新定义其映射的组合键, 也不会发生实际按键抬起事件未能监听到而造成消息不断弹出的bug情况。
