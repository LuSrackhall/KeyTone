...
### Requirement: 签名授权受理（UI 细节）

Normative: The signature management UI SHALL allow original authors to import authorization request files and review the request; in the review step the UI MUST show the requester's signature name and the matching local signature entry.

#### Scenario: 审核步骤展示申请方名称与本地签名卡片
