...
Normative: The system SHALL 提供授权策略对话（默认推荐无需授权）、风险提示对话及联系方式收集对话；当用户选择需要授权时，邮箱输入 MUST 校验格式且为空时禁用继续；当用户选择无需授权时，SHALL 提供可选联系方式填写对话框。

#### Scenario: 保持无需授权

- **GIVEN** 授权策略对话默认选择"无需授权"
- **WHEN** 用户直接点击"继续"
- **THEN** 状态机进入可选联系方式填写步骤

#### Scenario: 无需授权时填写联系方式

- **GIVEN** 用户选择"无需授权"后进入可选联系方式对话框
- **WHEN** 用户输入邮箱（可选附加信息）并点击"保存并继续"
- **THEN** 状态机记录联系方式并进入签名选择

#### Scenario: 无需授权时跳过联系方式

- **GIVEN** 用户选择"无需授权"后进入可选联系方式对话框
- **WHEN** 用户点击"跳过"
- **THEN** 状态机不记录联系方式，直接进入签名选择
...
Normative: The optional contact dialog SHALL 在用户选择"无需授权"后显示，提供可选的邮箱和附加联系方式输入；用户可直接跳过或填写后继续；如填写邮箱则需校验格式。

#### Scenario: 填写有效联系方式

- **GIVEN** 可选联系方式对话框可见
- **WHEN** 用户输入有效邮箱并点击"保存并继续"
- **THEN** 联系方式被记录，进入签名选择
...
- **GIVEN** 可选联系方式对话框可见
- **WHEN** 用户点击"跳过"
- **THEN** 不记录联系方式，直接进入签名选择

#### Scenario: 邮箱格式校验

- **GIVEN** 用户输入了邮箱但格式不正确
- **WHEN** 用户点击"保存并继续"
- **THEN** 显示格式错误提示，禁用继续操作
