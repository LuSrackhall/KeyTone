...
- 对话框关闭时应立即停止前端试听音频，避免“关闭后仍播放”的不符合直觉行为。
- 为保证关闭动画流畅，波形组件在对话框关闭时优先隐藏（v-show）而不是立即销毁（v-if）。
...
- 在波形图中间提供“横向音量指针条”，用户可通过上下拖动来快捷调整当前裁剪的音量（cut.volume）。
  ...
  - UI 展示采用 dB（内部仍是 cut.volume）：
    - 可视范围 ±18 dB；左侧标尺 18/12/6/0/-6/-12/-18；右侧显示当前 dB（可超过 ±18）。
    - 标尺放在波形外侧（不占用波形宽度），必要时可适度扩展对话框宽度，但不得超过视口可视范围；
      在宽度受限场景下标尺应布局在对话框内部，避免负向定位溢出。
      标尺需靠近波形边界便于对齐观察。
    - 不需要绘制贯穿整个横向区域的网格线；仅需刻度数字（可配短刻度）靠近波形边界。
    - 超出范围时指示线贴边并给出普通提示（非错误/警告）。
...
- 缩放体验打磨：`Ctrl + 鼠标滚轮` 快捷缩放（若启用）应当平滑、可控，并尽量以鼠标位置为锚点，避免缩放时视图跳动。
  - macOS 提示可显示为 Control/Command + 滚轮或触控板捏合，并优先识别 Control 修饰键以兼容外接键盘。
  - 若 wheel 事件未反映 ctrlKey，可在 window 层跟踪 Control/Command 按键状态作为兜底。
  - 快捷缩放 SHOULD 对 zoom 提交做帧级合并（RAF debounce），并避免 Shadow DOM 修复反馈回路，以减少低频闪烁。
  - 如仍存在极低频闪烁，可在 wheel burst 期间对 zoom 重绘限频，并在 burst 结束时补一次最终提交。
  - 必要时可在 wheel burst 期间对波形目标宽度做像素步长量化，减少 canvas 尺寸变化次数，从而进一步降低低频闪烁。
...
- 拖拽体验一致性：播放头拖拽与选区两侧指针拖拽也采用“边缘触发”自动滚动（指针靠近边缘才滚动、速度渐进），与右键快捷选区保持一致手感。
  ...
  - 播放头显示优先使用自定义 overlay，避免内置 cursor 在高频滚动时出现像素级抖动。
  - 播放中对 getCurrentTime 做单调递增约束，避免微小回跳导致播放头高频颤动。
  - 边缘推挤状态下锁定播放头位置（例如 85% 阈值线），由背景滚动承载运动，确保视觉稳定。
  - 边缘推挤接近尾部时解除播放头锁定，保持自然的播放滚动速度直到末尾。
  - 用户手动拖拽/点击播放头时应立即更新播放头显示；点击停止后播放头应立即回到 0 并滚动至起点。
  - 用户手动滚动波形时应同步更新播放头显示。
  - 在暂停/停止后首次右键拖拽时可进行一次轻量 reRender，确保选区过程可见。
  - 播放过程中右键拖拽选区不应冻结播放头 UI；播放头可移出视野并显示边缘指示。
  - 手动交互期间（右键选区/拖动选区/拖音量/滚动条滚动等）应以手动视区为主：暂停推挤/翻页，但播放头持续更新并可移出视野。
  - 边缘指示仅需变短变浅，不需要变颜色。
  - 自动跟随滚动的 scrollLeft 写入不应被误判为“用户滚动”，否则会造成 edge-push/paged-jump 视觉跳动。
  - 点击/拖拽滚动条时不应触发 seek；手动交互结束后恢复自动跟随应使用追帧动画平滑回到实时位置。
  - 缩放锚点修正需分离旧/新 zoom 坐标：旧 zoom 用于定位锚点，新 zoom 用于计算目标滚动位置。
  - 若存在 wheel 预计算锚点（pending anchor），应优先消费该锚点，避免时序竞争导致光标跟随漂移。
  - 对 Ctrl+滚轮缩放可增加基于同一 clientX 的时间域迭代补偿，进一步降低“放大后光标不在原波峰”的概率。
  - 对连续 Ctrl+滚轮缩放应固定 burst 锚点（timeSec + clientX）并暂停自动跟随竞争，确保放大链路围绕同一波峰收敛。
  - 为实现“锁同一波峰”的极致稳定性，wheel burst 锚点可基于采样索引（整数）建立，并在光标像素列对应窗口内选取最大振幅 sample。
...
- 在设置页新增“键音专辑页相关”分类，提供“波形滚动行为”选项：
  - **分页式跳转 (paged-jump)**：默认。播放头过右边缘后视口整体翻页，传统剪辑软件风格。
  - **智能边缘推挤 (edge-push)**：播放头在 0%~85% 区域自由移动，接近右缘时视口平滑跟随。
- 设置值通过 setting-store 持久化，应用重启后保持。
- UI 风格与设置页现有项保持一致（q-item + 左侧竖条 + q-select）。
