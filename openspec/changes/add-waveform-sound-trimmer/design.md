...
- 在暂停/停止后首次右键拖拽时，可执行一次轻量 reRender（no-op zoom）以确保选区过程可见。
...
用户对播放时波形的滚动方式有不同偏好，因此提供可配置选项。

### 模式说明

#### 分页式跳转 (paged-jump) - 默认

- 播放头在当前视口内自由移动，直到完全离开右侧边缘。
- 当播放头超出视口时，视口整体跳转为下一页（播放头出现在左侧 5%）。
- 优点：播放头运动感强，波形背景大部分时间保持静止，始终稳定。

#### 智能边缘推挤 (edge-push)

- 播放头在 0%~85% 区域自由移动（波形背景静止）。
- 当播放头接近右侧 85% 时，背景开始平滑推挤，播放头被“钉”在 85% 位置。
- 抛弃旧的“双向比较”算法，采用“单向推挤”：只当 idealLeft > currentScrollLeft 时才前推，消除浏览器 scrollLeft 整数舍入导致的来回抖动。

### 播放头稳定性

- 波形滚动时，WaveSurfer 内置 cursor 可能出现高频抖动（浮点时间 vs 整数 scrollLeft）。
- 为保证稳定视觉反馈，前端使用自定义播放头 overlay，基于滚动后的像素位置单独渲染。
- WaveSurfer 内置 cursor 设为隐藏，仅作为交互逻辑保留。
- 播放中对 `getCurrentTime()` 做单调递增约束（lastPlayheadSec），避免微小回跳引发抖动。
- 播放头位置采用整数像素量化（x 与 scrollLeft 同步取整），避免亚像素噪声。
- 边缘推挤时将播放头锁定在阈值线（例如 85% 位置），由背景滚动承载移动，避免相对抖动。
- 边缘推挤接近尾部时解除播放头锁定，让播放头自然移动到末尾；不引入额外“强制跳转”。
- 用户手动点击/拖拽播放头时，应立即更新自定义播放头位置，确保可见且跟手。
- 点击“停止”后播放头应立刻回到 0 位置，并将视口滚动到起点（不应等待二次操作）。
- 用户手动滚动波形时，应同步更新播放头位置，避免播放头固定在视野某一位置。
- 当播放头移出可视区域时，UI 应显示边缘指示（变色/缩短）提示其位于屏幕外。
- 手动交互（右键选区/拖动选区/拖音量/滚动条滚动等）期间，播放自动滚动（推挤/翻页）SHOULD 暂停，以手动视区为主；但播放头 UI MUST 持续更新并允许移出视野。
- 边缘指示不需要变颜色，仅通过“变短 + 变浅”提示其在视野外。
- 实现必须区分“用户滚动”和“程序性自动跟随滚动”：自动跟随写入 scrollLeft 不得触发用户滚动模式，否则 edge-push/paged-jump 会出现视觉跳动。
- 点击/拖拽滚动条（gutter/track/thumb）属于滚动意图，不应透传为波形内容区的 seek/拖拽（避免播放头跳到点击位置）。实现 SHOULD 同时拦截 pointerdown 与 mousedown（部分第三方实现以 mousedown 为主）。
- 手动交互结束后，恢复自动跟随时 SHOULD 使用短促的追帧动画（加速滚动追到实时位置），避免瞬间跳回造成突兀。
- 当检测到用户在滚动条区域按下并拖拽时，系统 SHOULD 显式进入“滚动条拖拽模式”，在 mouseup/pointerup 前保持手动视区优先，避免与自动跟随竞争闪烁。
- 追帧动画推荐使用带速度状态的弹簧阻尼（spring-damper），获得更“慢、更有惯性”的体验。
- 滚动条拖拽模式下的播放头刷新逻辑 SHOULD 尽量轻量，避免高频布局读取导致手动滚动出现明显步进/卡顿。
- 缩放锚点稳定性实现 SHOULD 使用“单一管线”处理：wheel 与 slider 的锚点都由 zoom watch 统一消费，避免双路径写入 scrollLeft 造成竞争抖动。
- 缩放后应执行一次“锚点误差二次补偿”：基于实际 scrollLeft 回读计算偏移误差并微调，消除极小概率漂移。
- 缩放 watch 处理中 SHALL 区分旧/新 zoom 坐标系：锚点推导使用旧 zoom，目标滚动计算使用新 zoom，避免坐标系混用引发漂移。
- 若存在 wheel 预计算锚点（pending anchor），系统 SHOULD 无条件优先消费，不应依赖异步标记状态。
- 对于 wheel 光标锚点，系统 SHOULD 基于同一 clientX 做时间域误差回推（可迭代 1~2 次），以消除高倍缩放下的残余漂移。
- 对于连续 wheel 缩放（burst），系统 SHOULD 锁定同一锚点（timeSec + clientX）直到本次缩放结束，避免每帧重采样锚点导致累积偏移。
- 对于“锁同一波峰”的精度要求，wheel burst 锚点 MAY 使用采样索引（整数）进行锚定，并可在当前像素列对应的时间窗内选择绝对振幅最大 sample 作为锚点。
- wheel 锚定窗口内，自动跟随滚动（推挤/翻页）SHOULD 暂停，避免与缩放修正竞争。
- 若缩放过程中仍出现极低频闪烁，系统 MAY 在 Ctrl+滚轮 burst 期间对 zoom 重绘进一步限频（例如约 20~30fps），并在 burst 结束时补一次最终提交以确保状态精确。
- 为避免缩放过程轻微抖动，滚动修正 SHOULD 尽量在一次提交内完成（基于实际 scrollWidth 解出 scrollLeft，必要时最多补一次），并在 wheel 场景用固定 clientX 重新计算 xInView。
- 若仍有残余低频闪烁，系统 MAY 在 wheel burst 期间对波形目标宽度做像素步长量化（并可随 zoom 自适应变大，例如 12/24/36px），减少 canvas 尺寸反复变化；burst 结束后恢复精确宽度。

### 对话框关闭行为

- 对话框关闭时应立刻停止前端试听音频，避免“关闭后仍播放”。
- 为避免销毁组件导致关闭动画卡顿，波形组件在对话框关闭时使用 v-show 隐藏而非 v-if 销毁。
- 通过对话框 before-hide 回调主动停止播放，保证动画与停止行为一致。
- 额外兜底：监听 dialog v-model 变化，确保任意关闭路径都能触发 stopPlayback。

### 持久化链路

1. 用户在设置页修改 -> `setting_store.keytoneAlbumScrollBehavior` 响应式更新
2. watch 触发 `StoreSet('keytoneAlbumScrollBehavior', val)` 写入持久化
3. 下次启动时 `getConfigFileToUi()` 从 `settingStorage.keytoneAlbumScrollBehavior` 读取并恢复
4. WaveformTrimmer 组件通过 computed 实时响应当前设置值

### UI 实现

- 设置页新增“键音专辑页相关”分类，展开后显示“波形滚动行为”下拉选择。
- 视觉风格与现有设置项一致：q-item + 左侧竖条装饰线 + q-select（borderless + dense）。
