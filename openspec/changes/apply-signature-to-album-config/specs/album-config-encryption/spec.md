...
### Requirement: 授权标识UUID管理

Normative: Original author signatures SHALL include an `authorizationUUID` field in the authorization object, generated by frontend nanoid on first export when "need signature" is selected (regardless of authorization choice), preserved on re-exports; this UUID is reserved for future signature authorization export/import functionality.

#### Scenario: 首次导出生成授权标识UUID

- **GIVEN** 用户首次导出专辑并选择"需要签名"
- **WHEN** 系统调用apply_signature_config
- **THEN** authorization对象包含authorizationUUID字段，值为前端nanoid生成的唯一标识

#### Scenario: 再次导出保留授权标识UUID

- **GIVEN** 专辑已有签名且authorization对象包含authorizationUUID
- **WHEN** 用户再次导出专辑
- **THEN** 原有的authorizationUUID保持不变，不被覆盖

#### Scenario: 无需授权时仍生成授权标识UUID

- **GIVEN** 用户首次导出专辑选择"需要签名"但选择"无需授权"
- **WHEN** 系统写入签名数据
- **THEN** authorization对象仍包含authorizationUUID字段，供未来授权功能使用
