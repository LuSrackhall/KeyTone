...
**流程变更**：
- 旧流程：再次导出确认 → 授权门控对话框 → 签名选择
- 新流程：再次导出确认 → 签名选择（带"导入授权"按钮）

**设计理由**：
- 若再次导出的用户是原始作者，固定出现的授权门控对话框没有意义
- 用户只能选择已授权的签名，无需强制进入授权门控
- 在签名选择页面提供"导入授权"按钮，用户可以主动导入授权文件

**"导入授权"按钮**：
- 显示条件：仅在再次导出且 `requireAuthorization=true` 时显示
- 位置：在"创建签名"按钮旁边
- 点击行为：关闭签名选择对话框，打开授权门控对话框
- 授权成功后：返回签名选择对话框，签名列表会刷新以反映新的授权状态
...
#### SignatureAuthorsDialog.vue（需求4）- 已重构
...
- 分区卡片式布局：原始作者（琥珀色）、直接导出作者（蓝色）、历史贡献作者（绿色）
- **原始作者区块完整展示**：
  - 签名卡片（图片 + 名称 + 介绍）
  - 资格码（带复制按钮）
  - 联系方式（邮箱、其他联系方式）
  ...
  - 直接导出作者资格码
  - 已授权签名列表（展开/折叠）
- **直接导出作者区块**：仅在与原始作者不同时显示
- **历史贡献作者区块**：列表形式，每项带复制按钮
- **签名统计摘要**
- 所有关键信息支持一键复制
- 横向滚动条样式与签名列表一致
- 处理无签名、加载中、错误状态（带重试按钮）
