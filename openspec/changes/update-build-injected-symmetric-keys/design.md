...
- SDK keys
  - `sdk/signature/encryption.go`：KeyA/KeyB（const -> var + Default + 解混淆）
  - `sdk/signature/album.go`：album signature inner key（const -> var + Default + 解混淆）
  - `sdk/audioPackage/enc/enc.go`：`FixedSecret`（const -> var + Default + 可变长度解混淆 + `GetFixedSecret()`）
  - `sdk/server/server.go`：`.ktalbum` v1/v2 XOR keys（const -> var + Default + 解混淆 + 版本选择/回退），并移除无意义硬编码清理 key 调用

- Build-time injection
  - `sdk/setup_build_env.sh`：扩展 `KEYS_TO_PROCESS` 映射
  - `sdk/private_keys.template.env`：增加 KEY_A/KEY_B/KEY_V1/KEY_V2/FIXED_SECRET/KEY_ALBUM_SIG

- ktalbum-tools
  - `tools/ktalbum-tools/utils/header.go`：v1/v2 key + 注入支持 + 候选 key 列表
  - `tools/ktalbum-tools/commands/info.go` / `extract.go`：按候选 key 解密并 checksum 验证
  - `tools/ktalbum-tools/setup_build_env.sh`：生成 ktalbum-tools 的 `EXTRA_LDFLAGS`
  - `tools/ktalbum-tools/build.sh`：构建时应用 `EXTRA_LDFLAGS`

### Quick Verification

- SDK（仅涉及改动包的编译/测试）：
  - `cd sdk && go test ./signature ./server ./audioPackage/enc`
- ktalbum-tools：
  - `cd tools/ktalbum-tools && go test ./...`
- OpenSpec 变更校验：
  - `openspec validate update-build-injected-symmetric-keys --strict`
