...
- [x] 1.10 导入重新加密：修改 `UpdateAlbumUUID` 支持传入原 UUID，自动解密旧配置、更新 `audio_pkg_uuid`、重新加密新 `core`；调整导入新专辑 API 传递原 UUID。
- [ ] 1.11 设计并实现旧版十六进制密文到 `core` 的迁移策略（加密 API 支持）。
- [ ] 1.12 单元测试：密钥派生、加解密往返、降级加载、写回、CLI、迁移、导入导出。
- [ ] 1.13 文档与示例更新：`TESTING.md`、README 等记录 `core` 文件格式与调试方法。
- [ ] 1.14 运行 `go test` 并验证无回归；lint 构建通过。
...
- [ ] 验证导入新专辑时，加密配置自动使用原 UUID 解密、新 UUID 加密。
- [ ] 验证覆盖导入加密专辑后能正常加载且不产生认证错误。

## 3. 文档

- [ ] 更新 changelog 添加功能摘要。
- [ ] 在 README 中记录调试 CLI 用法与 `core` 存储说明。
- [ ] 在 `TESTING.md` 中补充导入重新加密场景的说明。
