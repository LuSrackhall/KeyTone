...
- [x] 缩放锚点抗抖：统一 wheel/slider 锚点提交管线并增加二次误差补偿，修复偶发漂移。
- [x] 缩放坐标系一致性：watch 中分离 old/new zoom 坐标并优先消费 pending 锚点，进一步修复光标跟随漂移。
- [x] wheel 锚点迭代补偿：基于同一 clientX 的时间域回推修正，减少高倍缩放残余偏移。
- [x] wheel burst 锚点锁定：连续滚轮缩放期间固定同一锚点并暂停自动跟随竞争。
- [x] 采样索引锚定：wheel burst 首帧以采样索引（窗口内最大振幅）建立锚点，锁同一波峰。
- [x] 快捷缩放防闪烁：zoom 提交改为 RAF 合并（每帧最多一次），降低重绘频率。
- [x] 低频闪烁收敛：Shadow DOM 修复器恒定样式仅写一次，并在主动修复时抑制 observer 反馈回路。
- [x] 缩放重绘限频：wheel burst 期间对 zoom 重绘限频（约 30fps）并在结束时补一次最终提交。
- [x] 缩放宽度量化：wheel burst 期间量化波形目标宽度，减少 canvas 尺寸变化引发的残余闪烁。
