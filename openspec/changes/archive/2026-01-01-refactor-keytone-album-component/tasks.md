...
| Phase 2 | ✅ 完成 | Step1/2/3/4 已替换为独立组件（父组件薄壳 + inject UI） |
| Phase 3 | ✅ 完成 | 6 个核心 Dialog 组件已创建并集成               |
| Phase 4 | ✅ 完成 | composables/mappers 已落地并复用（含 initData/watch） |
...
- [x] 定义组件结构，通过 inject 获取 Context
- [x] 添加详细架构注释
- [x] 用子组件替换父组件中的原有 Step4 模板（确保行为一致）

> **说明**: Step4 迁移遵循“模板优先”策略：父组件继续持有所有状态与方法，通过 Context 提供给 `StepLinkageEffects`。
...
- [x] 5.5 完成最小手动回归路径（用户已简单测试）
- [x] 5.6 删除父组件中旧的 `v-if="false"` 单键声效对话框实现（仅保留新组件）
...
**当前状态**:
- ✅ Step1/2/3 已替换为独立组件
- ✅ Step4 已替换为独立组件（`StepLinkageEffects` 承载 UI；状态与副作用仍在父组件）
- ✅ 单键/全键声效对话框已抽离并集成（SingleKeyEffectDialog/EveryKeyEffectDialog）
- ✅ 已删除父组件中旧的单键声效对话框实现（原先用于对照的 `v-if="false"` 块）
...
**可选后续工作**:
- 将 Step4 内部更复杂的 UI 继续细拆（例如虚拟键盘区域独立组件等），但需谨慎回归验证
...
| 日期       | 变更内容                                            |
| ---------- | --------------------------------------------------- |
| 2025-12-31 | 删除父组件中旧的单键声效对话框实现（v-if=false 块） |
