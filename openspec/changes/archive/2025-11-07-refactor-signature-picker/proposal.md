...
- **当前问题 1**：卡片布局不紧凑，图片占据顶部 80px 高度，导致列表显示效率低下
- **当前问题 2**：签名数据为硬编码 mock 数据，无法反映真实系统状态
- **当前问题 3**：排序逻辑分离，签名管理页面与选择对话框的数据不同步
- **改进机会**：参考签名管理页面的成熟设计（左侧图片 60px + 右侧文本区），优化视觉效率；通过接入真实数据和 SSE 推送，保持数据一致性

## What Changes

- **UI 布局**：改进 `SignaturePickerDialog` 组件的卡片布局，从列式（图片顶部）改为行式（图片左侧），提升紧凑度
- **数据源**：接入真实签名数据，从签名管理 store 加载而非依赖 mock prop，支持 SSE 增量更新
- **保持兼容**：保留 `signatures` prop 和现有事件签名，确保向后兼容
