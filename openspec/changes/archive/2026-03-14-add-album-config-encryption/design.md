...
- 保持相同文件名以避免修改枚举 `package.json` 的现有代码；指示 JSON 与源 `core` 文件分离。
- 直接使用 SHA256 而非 PBKDF2（简化；无用户提供的密码）。
- 导入新专辑时自动重新加密：使用原 UUID 解密，使用新 UUID 加密，避免认证失败。

## 考虑过的替代方案

- 单独扩展名（如 `package.enc.json`）—— 被拒绝（改动过于侵入）。
- 使用 PBKDF2 迭代 —— 被拒绝（不必要的复杂性；固定密钥已限制收益）。
- 导入时保留原 UUID 的加密 —— 被拒绝（导致认证失败；重新加密更健壮）。
...
- 导入重新加密失败：已实现回退逻辑（尝试原 UUID，失败则尝试目录名）；提供合理的错误日志。

## 迁移计划

1. 实现密钥派生辅助函数和加解密工具。（完成）
2. 在 `LoadConfig` 中添加检测和降级。（完成）
3. 实现二进制 `core` 存储和指示 JSON。（完成）
4. 集成独立加密 API 路由供前端调用（支持旧版十六进制迁移）。（完成）
5. 修改 `UpdateAlbumUUID` 支持密钥重新派生和重新加密。（完成）
6. 提供 CLI 和测试。（进行中）
7. 发布；明文专辑继续工作；加密专辑自动加载；导入新专辑自动重新加密。

## 待确认问题

- 是否在降级触发时记录日志？（是，在 INFO 级别记录一次。）
- 导出后是否保留原始明文？（否；用加密内容覆盖。）
- 导入新专辑时是否支持 UUID 重新加密？（是，自动识别并执行。）
