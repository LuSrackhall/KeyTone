...
- **视觉设计**：当前卡片设计扁平简陋，缺乏现代感和视觉层次感，需要通过阴影、hover 和 selected 状态优化
- **交互体验**：底部按钮随内容滚动不符合常见 UI 模式，应固定在底部；选中后无法取消选择，用户操作不灵活
- **内容显示**：搜索区域过大、卡片间距过大、主滚动条占用空间，导致整体布局不够紧凑
- **改进机会**：参考签名管理页面的成熟设计模式，优化间距、固定操作栏、隐藏滚动条，提升用户体验

## What Changes

- **视觉增强**：改进卡片阴影、hover 效果、selected 状态的视觉层次，添加光晕动画
- **交互优化**：支持点击已选项取消、固定底部操作栏、隐藏主滚动条，改进用户流程
- **布局优化**：调整间距（搜索区 padding、卡片间距、底部 padding），内容支持横向滚动，提升紧凑度
