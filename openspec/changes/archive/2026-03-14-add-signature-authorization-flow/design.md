...
- 文本溢出处理（AuthGrantDialog + AuthRequestDialog）：
  - 所有签名名称、介绍、申请方名称等长文本字段均采用 `overflow-x-auto` + `white-space: nowrap` 实现横向滚动，避免对话框整体溢出。
  - 横向滚动条样式 MUST 与签名管理页签名列表的横向滚动条保持一致（复用相同的细滚动条样式规则）。
