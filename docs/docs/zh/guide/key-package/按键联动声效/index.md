...
## **鼠标按键音效支持** <Badge type="tip" text="新功能" />

除了键盘按键，KeyTone 现在支持为**鼠标按键事件**绑定音效：

鼠标按键项与键盘按键的实际绑定方式完全一致。可为它们分配任意音频源文件、定义声音或至臻键音，使用与键盘按键相同的优先级覆盖逻辑。

## **键盘/鼠标专辑分离模式** <Badge type="tip" text="新功能" />

默认情况下，单一选中的专辑同时用于**键盘与鼠标**事件（统一模式）。KeyTone 还提供了可**分别选择键盘专辑与鼠标专辑的分离模式**。

* 在主页面即可切换到分离模式。
* 分离模式下，主页面出现两个独立的专辑选择器：键盘一个、鼠标一个。
* 每个选择器独立控制播放、音量与静音状态。 换句话说, 分离模式下，键盘与鼠标各自拥有**独立音量控制**，与全局音量滑块叠加生效。
* **回退行为**：鼠标专辑未设置时，默认使用内嵌测试音。可在设置中开启“鼠标回退至键盘专辑”选项——开启后, 若分离模式下鼠标专辑缺失时，可回退到键盘专辑而非内嵌测试音。

此模式适合希望将**键盘音效专辑**与**鼠标点击专辑**分开配置的用户, 允许用户自由组合搭配不同专辑的键盘和鼠标配置以满足更加个性化的使用需求。