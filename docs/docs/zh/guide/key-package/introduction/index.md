...
在此基础上，KeyTone 引入了完整的**专辑签名与授权体系**：

* **作者签名**：专辑创作者可创建一个或多个签名以对专辑进行签名。有效的签名证明了专辑的来源，并证明签名后专辑未被篡改，还能在一定程度上保护创作者的版权。
* **配置加密**：`.ktalbum` 内部的专辑配置数据已加密处理，防止未经授权的检视或修改。
* **导入验证**：无论是否签名, 都不会影响导入操作, 不过若有签名的专辑导入或应用后，KeyTone 会清晰展示专辑是否已签名、签名人身份以及签名是否有效——帮助用户做出更加明智的信任判断。
